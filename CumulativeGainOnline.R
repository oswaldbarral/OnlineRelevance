...
#generate grand average of bookmarked articles per condition
# ga_dataBM <- lapply(split(data,data$Condition), function (d){
#   r <- unlist(lapply(split(d,d$Iteration),function(d1){
#     mean( unlist(lapply(split(d1$Bookmarks_perIt,d1$PID),function(x) x[1])))
#   }))
# })
# #generate raw bookmarked articles per condition
# res <- lapply(split(data,data$Condition), function (d){
#     r <- lapply(split(d,d$Iteration),function(d1){
#       data.frame("BA" = unlist(lapply(split(d1$Bookmarks_perIt,d1$PID),function(x) x[1])))
#     })
#     r1 <- do.call(rbind,r)
#     r1$Iteration <- unlist(lapply(c(1:10),rep,length(r[[1]]$BA)))
#     return (r1)
# })
# raw_dataBM <- do.call(rbind,res)
# raw_dataBM$Condition <- factor(unlist(lapply(c("Full System","Baseline"),rep,length(res[[1]]$BA))))


# 
# low_th <- min(min(ga_dataBM$`3`),min(ga_dataBM$`1`))
# high_th <- max(max(ga_dataBM$`3`),max(ga_dataBM$`1`))
# plot(ga_dataBM$"1",col=1,type="l",ylim = c(low_th,high_th),ylab = "Bookmarked articles", xlab = "Iteration",xaxt="n")
# axis(1,at=c(1:10),labels=c(1:10))
# legend("topright", inset=.05, cex = 1, c("baseline","full system"), lty=c(1,1), lwd=c(2,2), col=c(2,1), bg="grey96")
# lines(ga_dataBM$"3",col=2)
# 
#Plot grand average of relevant items on screen per condition
# ga_dataRElItems <- lapply(split(data,data$Condition), function (d){
#   r <- unlist(lapply(split(d,d$Iteration),function(d1){
#     mean( unlist(lapply(split(d1$RelevantKeywordsBIN,d1$PID),function(x) x[1])))
#   }))
# 
# })
# low_th <- min(min(ga_dataRElItems$`3`),min(ga_dataRElItems$`1`))
# high_th <- max(max(ga_dataRElItems$`3`),max(ga_dataRElItems$`1`))
# plot(ga_dataRElItems$"1",col=1,type="l",ylim = c(low_th,high_th),ylab = "Relevant keywords", xlab = "Iteration",xaxt="n")
# axis(1,at=c(1:10),labels=c(1:10))
# legend("topright", inset=.05, cex = 1, c("baseline","full system"), lty=c(1,1), lwd=c(2,2), col=c(2,1), bg="grey96")
# lines(ga_dataRElItems$"3",col=2)


#Plot grand average of keyword scores per condition
# ga_dataRElItems <- lapply(split(data,data$Condition), function (d){
#   r <- unlist(lapply(split(d,d$Iteration),function(d1){
#     mean( unlist(lapply(split(d1$RelevantKeywords,d1$PID),function(x) x[1])),na.rm=T)
#   }))
# 
# })
# low_th <- min(min(ga_dataRElItems$`3`),min(ga_dataRElItems$`1`))
...
# 
# #Plot grand average of time spent on screen per condition
# ga_dataTime <- lapply(split(data,data$Condition), function (d){
#   r <- unlist(lapply(split(d,d$Iteration),function(d1){
#     mean( unlist(lapply(split(d1$Time_perIt,d1$PID),function(x) x[1])))
#   }))
# 
# })
# low_th <- min(min(ga_dataTime$`3`),min(ga_dataTime$`1`))
# high_th <- max(max(ga_dataTime$`3`),max(ga_dataTime$`1`))
# plot(ga_dataTime$"1",col=1,type="l",ylim = c(low_th,high_th),ylab = "Relevant keywords", xlab = "Iteration",xaxt="n")
# axis(1,at=c(1:10),labels=c(1:10))
# legend("topright", inset=.05, cex = 1, c("baseline","full system"), lty=c(1,1), lwd=c(2,2), col=c(2,1), bg="grey96")
# lines(ga_dataTime$"3",col=2)
...
# # #Plot overall Time
# # d$TotalTime <-  unlist(lapply(d$BookmarkedArticles,function(x){
# #   aux <- strsplit(x,"-")[[1]]
# #   mean(as.numeric(aux)[1:length(aux)-1])
# # }))
# # pl1 <- ggplot(d, aes(x=Condition, y=TotalTime, fill=Condition)) + geom_boxplot() + guides(fill=FALSE)
# # print(pl1)
